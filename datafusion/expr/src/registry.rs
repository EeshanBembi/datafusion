...
use arrow_schema::extension::{
    Bool8, ExtensionType, FixedShapeTensor, FixedShapeTensorMetadata, Json, JsonMetadata,
    Opaque, OpaqueMetadata, TimestampWithOffset, VariableShapeTensor,
    VariableShapeTensorMetadata,
};
use datafusion_common::types::{DFExtensionType, DFExtensionTypeRef};
use datafusion_common::{
    DataFusionError, HashMap, Result, not_impl_err, plan_datafusion_err,
};
...
impl MemoryExtensionTypeRegistry {
    ...
    pub fn new_with_canonical_extension_types() -> Self {
        let mapping: [ExtensionTypeRegistrationRef; 7] = [
            DefaultExtensionTypeRegistration::new_arc(|_, _| {
                Ok(arrow_schema::extension::Uuid {})
            }),
            DefaultExtensionTypeRegistration::new_arc(|_, _: &'static str| Ok(Bool8 {})),
            DefaultExtensionTypeRegistration::new_arc(|dt, metadata: JsonMetadata| {
                <Json as ExtensionType>::try_new(dt, metadata)
                    .map_err(DataFusionError::from)
            }),
            DefaultExtensionTypeRegistration::new_arc(|_, metadata: OpaqueMetadata| {
                Ok(Opaque::from(metadata))
            }),
            DefaultExtensionTypeRegistration::new_arc(
                |dt, metadata: FixedShapeTensorMetadata| {
                    <FixedShapeTensor as ExtensionType>::try_new(dt, metadata)
                        .map_err(DataFusionError::from)
                },
            ),
            DefaultExtensionTypeRegistration::new_arc(
                |dt, metadata: VariableShapeTensorMetadata| {
                    <VariableShapeTensor as ExtensionType>::try_new(dt, metadata)
                        .map_err(DataFusionError::from)
                },
            ),
            DefaultExtensionTypeRegistration::new_arc(|_, _| Ok(TimestampWithOffset {})),
        ];
